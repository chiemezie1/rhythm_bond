...
export default function PlaylistShareModal({ playlist, isOpen, onClose, onShare }: PlaylistShareModalProps) {
  const { user } = useAuth();
  ...
  const handleCreatePost = async () => {
    setIsSharing(true);
    try {
      // Create a post sharing this playlist
      const response = await fetch('/api/posts', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          content: message.trim() || `Check out my "${playlist.name}" playlist with ${playlist.trackCount} tracks!`,
          mediaType: 'playlist',
          mediaId: playlist.id,
          visibility: 'public'
        })
      ...
      <div className="bg-dark-lighter rounded-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold">Share Playlist</h2>
            <button
              onClick={onClose}
          ...
          <div className="bg-dark-lightest rounded-lg p-4 mb-6">
            ...
            <p className="text-primary text-sm mt-2">{playlist.trackCount} tracks</p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mb-3">
                {shareType === 'copy' 
                  ? 'Creates a copy of this playlist in their library'
                  : 'Shares a reference to your playlist'
                }
              </p>
              ...
              >
                {isSharing ? 'Sharing...' : `${shareType === 'copy' ? 'Copy' : 'Share'} Playlist`}
              </button>
            </div>
            ...
            <div>
              ...
              <textarea
                placeholder={`Check out my "${playlist.name}" playlist with ${playlist.trackCount} tracks!`}
                value={message}
                onChange={(e) => setMessage(e.target.value)}
